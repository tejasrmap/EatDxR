...
export function AppNavigationDock() {
  ...
  const isActive = (path: string) => {
    if (path === '/app' && (location.pathname === '/app' || location.pathname === '/app/')) return true;
  ...
  return (
    <>
      <nav className="fixed bottom-0 left-0 right-0 z-50 pointer-events-none pb-[env(safe-area-inset-bottom,4px)]">
        <div className="max-w-sm sm:max-w-md mx-auto px-3 sm:px-4 pb-1.5">
          <div className="w-full bg-zinc-950/92 backdrop-blur-2xl border border-white/10 rounded-full shadow-2xl pointer-events-auto p-1.5 flex items-center justify-between relative">
            
            {/* 1. Feed / Home */}
            <Link
              to="/app"
              onClick={() => triggerHaptic()}
              className={`flex-1 flex flex-col items-center justify-center py-1 rounded-full transition-all active:scale-90 ${
                isActive('/app') ? 'text-orange-400' : 'text-white/40 hover:text-white'
              }`}
            >
              <LayoutGrid size={19} className={isActive('/app') ? 'fill-orange-400/20 stroke-orange-400' : ''} />
            ...
            <Link
              ...
              onClick={() => triggerHaptic()}
              className={`flex-1 flex flex-col items-center justify-center py-1 rounded-full transition-all active:scale-90 ${
                isActive('/dishes') || isActive('/restaurants') || isActive('/map')
                  ? 'text-orange-400'
                  : 'text-white/40 hover:text-white'
              }`}
            >
              <Compass size={19} className={isActive('/dishes') ? 'fill-orange-400/20 stroke-orange-400' : ''} />
            ...
            <Link
              ...
              onClick={() => triggerHaptic()}
              className={`flex-1 flex flex-col items-center justify-center py-1 rounded-full transition-all active:scale-90 ${
                isActive('/lists') || isActive('/list') ? 'text-orange-400' : 'text-white/40 hover:text-white'
              }`}
            >
              <ListOrdered size={19} className={isActive('/lists') ? 'fill-orange-400/20 stroke-orange-400' : ''} />
            ...
            <button
              ...
              className={`flex-1 flex flex-col items-center justify-center py-1 rounded-full transition-all active:scale-90 cursor-pointer ${
                isActive('/app/profile') ? 'text-orange-400' : 'text-white/40 hover:text-white'
              }`}
            >
              <User size={19} className={isActive('/app/profile') ? 'fill-orange-400/20 stroke-orange-400' : ''} />
